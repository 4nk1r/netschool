...
private fun AppNavHost(navController: NavHostController) {
    ...
    AnimatedNavHost(
        ...
        bottomSheet(
            ...
            arguments = listOf(navArgument(NAV_ARG_ASSIGNMENT) {
                // custom nav type didn't work, waste of time
                type = NavType.Companion.StringType
            })
        ) { backStackEntry ->
            AssignmentInfoBottomSheet(
                assigns = Gson().fromJson(
                    backStackEntry.arguments!!.getString(NAV_ARG_ASSIGNMENT),
                    object : TypeToken<List<AssignmentParcelable>>() {}
                ).map { it.toDomain() },
                navController = navController
            )
        }
...
const val NAV_ARG_ASSIGNMENT = "assignment"